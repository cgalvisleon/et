...
> **Note:** Almost the entire repo has no `*_test.go` files — `go test ./...` mostly compiles and finds nothing to run. The one exception is `queue/example_test.go` (a runnable `Example_queue`); don't assume it generalizes to other packages.
...
**`Store` interface** (`jwf/store.go`): `Set(collection, id, ownerId string, obj any) error`, `Get(collection, id string, dest any) (bool, error)`, `Delete(collection, id string) error`, `Query(collection string, query et.Json) (et.Items, error)` — **no `GenSerie` method** (an earlier version of this doc claimed one was required; not true in the current code — `GenSerie` only exists, unrelated, as a method on `jsql.Series`). A default jsql-backed implementation ships as `jwf.Storage`, built via `jwf.DefineStore(db *jsql.DB, schema string) (*Storage, error)` (`jwf/store.go`), managing `"workflows"`/`"flows"`/`"steps"`/`"instances"` jsql models.
