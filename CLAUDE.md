...
- **`validator/`** — A second, separate fluent validation package (`Validator`/`Field`/`Condition`), with its own i18n'd (`LANG` env var) message constants in `validator/msg.go`. Overlaps in purpose with `jval/` but is a distinct, unrelated type — don't assume they share types or can be mixed. `Condition` (from `validator.New().Field(name)`) chains `.Required()`/`.Min()`/`.Max()`/`.Between()`/`.MinLength()`/`.MaxLength()`/`.Pattern()` plus `.NotEmpty()` (non-whitespace string) and three combinable content checks — `.IsLetters()`/`.IsNumbers()`/`.IsSpecialCharacters()` — each requiring only *at least one* character of that class, so stacking them expresses rules like password strength rather than an exclusive charset. Its internal `validate()` type-switches on `string`/`int`/`float64`/`bool`/`time.Time`/`[]byte`/`[]string`/`[]int`/`[]float64`/`[]any` — the `[]any` case matters because values from a real JSON-decoded `et.Json` arrive as `[]any`, not typed slices. `Validator` itself (not `Condition`) also has its own separate `.NotEmpty()` — an opt-in flag (default off) that makes `Validate` reject a totally empty `et.Json{}` up front, independent of any per-field `Required()`.
...
- **`infobip/`** — Infobip API client: SMS, email, WhatsApp (template messages) — no longer an empty stub. `infobip.NewSenderInfobip(infobip.Params{BaseUrl, ApiKey, Sender}) (*SenderInfobip, error)` builds one client shared by all three channels (`BaseUrl`/`ApiKey`/`Sender` all required, validated in the constructor). `(*SenderInfobip).SendSMS`/`SendEmail`/`SendWhatsApp` each take an explicit `tpMessage` string (`"Transactional"` or `"Promotional"`, validated) rather than exposing separate `...Transactional`/`...Promotional` wrapper functions the way `brevo` does. `SendSMS` (`sms.go`, pre-existing) posts to `/sms/3/messages`; `SendEmail` (`email.go`) posts JSON to `/email/4/messages` — deliberately the JSON-based Email API v4 endpoint, not the multipart-only `/email/3/send` v3 one, since this repo's shared `request.Fetch` helper only supports JSON/form-urlencoded bodies; `SendWhatsApp` (`whatsapp.go`) posts to `/whatsapp/1/message/template` once per recipient (pre-approved templates only, with positional `{{n}}` placeholders via `placeholders [][]string` indexed by recipient — free-form session messages aren't supported). All three read `params`/`htmlContent` template substitution via `{{key}}` replacement, same convention as `brevo`.
...
**HTTP routing:** `(*WorkFlow).LoadRouter(r Router)` wires 11 routes (`jwf/router.go`): 4 for Steps, 4 for Flows, 3 for Instances. Only the **Steps** sub-API is actually implemented: `httpGetStep`, `httpSetStep`, `httpUpdateStep`, `httpDeleteStep`. The **Flows** (`httpGetFlow`, `httpSetFlow`, `httpStatusFlow`, `httpDeleteFlow`) and **Instances** (`httpGetInstance`, `httpDeleteInstance`, `httpRunInstance`) handlers are registered but still have **empty function bodies** — not yet implemented. `Router` is a minimal local interface (`Protect(method, path string, handler func(http.ResponseWriter, *http.Request))`), unrelated to the repo's `router/` package.

**Reusable pattern:** the `AuditLog []et.Json` + `onSave`/`onDelete` hook pattern used by `Step` (`jwf/step.go`) and `Flow` (`jwf/flow.go`) — per-instance audit trail capped at `MAX_AUDIT_LOG` (env var, default 1000), plus registrable save/delete callback hooks — is documented as a copy-paste template in `AUDITLOG.md` at the repo root, for replicating onto other structs.
