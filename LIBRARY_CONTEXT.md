...
4. **Inversión de dependencias vía interfaces pequeñas y locales — hoy de facto unificadas**: la librería define interfaces de persistencia (`jsql.Driver`, `jia.Store`, `jwf.Store`, `resilience.Store`, `config.Store`, `crontab.Store`, `jrex.Store`) y el consumidor las implementa. No existe una interfaz `Store` compartida en Go (el antiguo paquete `instances/` fue eliminado, no se reemplazó por otro tipo común) — pero la lectura directa del código muestra que **`jia.Store`, `jwf.Store`, `resilience.Store`, `crontab.Store` y `jsql.Store` comparten hoy exactamente la misma forma**: `Set(collection, id, ownerId string, obj any) error` / `Get(collection, id string, dest any) (bool, error)` / `Delete(collection, id string) error` / `Query(query et.Json) (et.Items, error)` (`jwf.Store` añade solo `GenSerie(tag) (string, error)`). `jrex.Store` es un subconjunto intencional de 2 métodos (`Set`/`Get`, sin `Delete`/`Query`). `config.Store` es la excepción real: usa claves `(tag, stage)` en vez de `(collection, id)`, no tiene `Query`, y además tiene un bug confirmado (ver Anti-Patrones) que intercambia `tenantId`/`ownerId` al llamar `Set`.
...
- **Patrón Store inyectado, convergiendo hacia una forma única**: `jia.New(tag, store, userId)`, `jwf.New(store)`, `crontab.New(tag, store)`, `resilience.New(store)`, `jsql` (internamente), `jrex.Load(tag, store)` — cada uno declara su propio tipo `Store` local (no hay un tipo Go compartido), pero cuatro de ellos (`jia`, `jwf`, `resilience`, `crontab`) más `jsql` comparten **exactamente** la misma forma de método (ver Design Philosophy §4). Ni `jia.New` ni `jwf.New` reciben ya un `tenantId` (ambos generan su propio `ID` internamente con `reg.UUID()`; cargar una instancia existente es `jia.Load(id, store)` / `jwf.Load(id, store)`). La excepción real es `config.Store` (forma distinta + bug de parámetros cruzados) y los casi-calces de `stores.Catalog`/`stores.Instance` (ver Anti-Patrones).
...
- `crontab/` — rediseñado a un modelo orientado a eventos: `crontab.Load(tag, store) error` arranca un singleton de paquete (internamente llama `crontab.New(tag, store)`, que requiere `Store`); registra jobs con las funciones de paquete `crontab.CronJob(tag, ownerId string, spec Cron, repetitions int, params et.Json, fn) error` (recurrente, `Cron{DayOfWeek, Month, DayOfMonth, Hour, Minute}` estructurado, no un string crudo) o `crontab.ScheduleJob(tag, ownerId string, spec time.Time, params et.Json, fn) error` (una sola ejecución). Los antiguos `AddJob`/`AddOneShotJob`/`AddEventJob`/`StartJob`/`StopJob`/`DeleteJob` como métodos de instancia **ya no existen**; el control (stop/start/remove) ahora se hace publicando eventos (`crontab.HttpStopJob`, etc.) que un listener interno traduce a los métodos privados del singleton.
...
10. **No confíes en `config.Store` para distinguir `tenantId` de `ownerId`.** `(*config.Config).Save()` (`config/config.go:136`) llama `store.Set(s.Tag, s.Stage, s.OwnerId, s.TenantId, s)`, pero la interfaz declara el orden `Set(tag, stage, tenantId, ownerId string, obj any)` — los argumentos 3 y 4 están intercambiados. Si implementas un `config.Store` real, ten presente que recibirá esos dos campos cruzados.
11. **No copies ejemplos de `crontab` con `AddJob`/`AddOneShotJob`/`AddEventJob`/`StartJob`/`StopJob`/`DeleteJob` como métodos de instancia.** El paquete fue rediseñado a un modelo orientado a eventos con un singleton (`crontab.Load(tag, store)`) y funciones de paquete `crontab.CronJob(...)`/`crontab.ScheduleJob(...)`; esos métodos de instancia ya no existen.
...
| Cron                            | `crontab/`                                 | `robfig/cron` directo                                    | Si no necesitas el modelo orientado a eventos NATS (`CronJob`/`ScheduleJob` vía singleton) que añade `crontab/`                                                                                            |
...
| `ct.AddJob(...)` / `ct.AddOneShotJob(...)` / `ct.AddEventJob(...)` / `ct.StartJob/StopJob/DeleteJob(...)` (métodos de instancia) | `crontab.Load(tag, store)` (singleton de paquete) + `crontab.CronJob(tag, ownerId, spec Cron, repetitions, params, fn)` / `crontab.ScheduleJob(tag, ownerId, spec time.Time, params, fn)` (funciones de paquete) |
