...
func SetCtx(ctx context.Context, key, val string, second time.Duration) error {
	if conn == nil {
		return et.Nil(msg.T("ERR_NOT_CACHE_SERVICE"))
	}
...
func GetCtx(ctx context.Context, key, def string) (string, error) {
	if conn == nil {
		return def, et.Nil(msg.T("ERR_NOT_CACHE_SERVICE"))
	}
...
func DelCtx(ctx context.Context, key string) (int64, error) {
	if conn == nil {
		return 0, et.Nil(msg.T("ERR_NOT_CACHE_SERVICE"))
	}
...
func HGetCtx(ctx context.Context, key string) (map[string]string, error) {
	if conn == nil {
		return map[string]string{}, et.Nil(msg.T("ERR_NOT_CACHE_SERVICE"))
	}
...
func More(key string, second time.Duration) int {
	...
	val++
	Set(key, et.Format(`%d`, val), second)
...
func SetVerify(device, key, val string) error {
	key = et.Format(`verify:%s/%s`, device, key)
...
func AllCache(search string, page, rows int) (et.List, error) {
	if conn == nil {
		return et.List{}, et.Nil(msg.T("ERR_NOT_CACHE_SERVICE"))
	}
...
func GetJson(key string) (et.Json, error) {
	if conn == nil {
		return et.Json{}, et.Nil(msg.T("ERR_NOT_CACHE_SERVICE"))
	}
...
func SetJson(key string, val et.Json, second time.Duration) (et.Json, error) {
	if conn == nil {
		return val, et.Nil(msg.T("ERR_NOT_CACHE_SERVICE"))
	}
...
func GetItem(key string) (et.Item, error) {
	if conn == nil {
		return et.Item{}, et.Nil(msg.T("ERR_NOT_CACHE_SERVICE"))
	}
...
func GetItems(key string) (et.Items, error) {
	if conn == nil {
		return et.Items{}, et.Nil(msg.T("ERR_NOT_CACHE_SERVICE"))
	}
