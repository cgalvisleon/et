...
// new statement looks true or like a lie against what is already known. It can
// optionally be wired to a local LLM (see UseLLM) so Ask grounds its answers with
// real language generation instead of only template-based phrasing — Ask works
// exactly as before when no LLM is configured.
// @param manager *Manager, classifier *Classifier, llm LLM
type Engine struct {
	manager    *Manager
...
/**
* UseLLM: wires an optional LLM into the Engine so Ask can ground its answers with
* real language generation (retrieval-augmented generation over the facts already
* found by RelevantFacts) instead of only template-based phrasing. Pass nil to
* disable it again. Returns s for chaining.
* @param llm LLM
* @return *Engine
**/
func (s *Engine) UseLLM(llm LLM) *Engine {
	s.llm = llm
	return s
}
...
type AskResult struct {
	...
	// Source is "llm" when Answer was generated by the LLM configured via UseLLM,
	// and omitted for every heuristic/template-based answer (greeting, list-facts,
	// naturalAnswer, or the LLM being unavailable/unconfigured).
...
func (s *Engine) Ask(kbId, question string) (AskResult, error) {
	...
	if s.llm != nil {
		if result, ok := s.askLLM(question, best, related); ok {
			return result, nil
		}
	}
...
/**
* askLLM: attempts to answer question via s.llm, grounded on best plus related facts
* (retrieval-augmented generation). Returns ok=false on any failure — a Complete
* error, or an empty/whitespace-only response — so Ask falls back to its existing
* heuristic pipeline; the LLM is an enhancement layer, never a hard requirement.
* @param question string, best *Fact, related []*Fact
* @return AskResult, bool
**/
func (s *Engine) askLLM(question string, best *Fact, related []*Fact) (AskResult, bool) {
	facts := append([]*Fact{best}, related...)
	prompt := buildRAGPrompt(question, facts)

	answer, err := s.llm.Complete(context.Background(), prompt)
...
/**
* buildRAGPrompt: builds the grounding prompt askLLM sends to the LLM — an
* instruction to answer only from the given facts (in both plain-text and
* Subject/Predicate/Object form, see triple.go), followed by the question.
* @param question string, facts []*Fact
* @return string
**/
func buildRAGPrompt(question string, facts []*Fact) string {
	var b strings.Builder
	b.WriteString("Eres un asistente que responde preguntas basándote única y exclusivamente en los siguientes hechos conocidos. ")
	b.WriteString("Si los hechos no contienen información suficiente para responder, dilo claramente en vez de inventar algo. ")
	b.WriteString("Responde en español, en una o dos frases, de forma natural y directa.\n\n")

	b.WriteString("Hechos conocidos:\n")
	for i, fact := range facts {
		fmt.Fprintf(&b, "%d. %s", i+1, fact.Statement)
		for _, tr := range fact.Triples {
			fmt.Fprintf(&b, " [%s | %s | %s]", tr.Subject, tr.Predicate, tr.Object)
		}
		b.WriteString("\n")
	}
