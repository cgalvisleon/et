...
var (
	...
	MSG_STEP_DEFINITION_IS_UNKNOWN    = "step definition is unknown %v"
	...
	MSG_INSTANCE_STATUS               = "Instancia:%s flow:%s step:%s status:%s"
	MSG_INSTANCE_ERROR                = "Instancia:%s flow:%s step:%s error:%s"
...
func init() {
	...
	if lang == "es" {
		...
		MSG_INSTANCE_STATUS = "Instancia:%s flow:%s step:%d status:%s"
		MSG_INSTANCE_ERROR = "Instancia:%s flow:%s step:%d error:%s"
