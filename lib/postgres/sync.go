...
func defineSync(db *sql.DB) (string, error) {
  ...
  CREATE INDEX IF NOT EXISTS SYNCS__IDT_IDX ON core.SYNCS(_IDT);
	CREATE INDEX IF NOT EXISTS SYNCS__STATE_IDX ON core.SYNCS(_STATE);
  ...
  CREATE INDEX IF NOT EXISTS SYNCS__SYNC_IDX ON core.SYNCS(_SYNC);
	CREATE INDEX IF NOT EXISTS SYNCS__INDEX_IDX ON core.SYNCS(_INDEX);
  ...
  BEGIN
    IF NEW._IDT = '-1' THEN
      ...
      INSERT INTO core.SYNCS(TABLE_SCHEMA, TABLE_NAME, _IDT, _STATE, ACTION)
      VALUES (TG_TABLE_SCHEMA, TG_TABLE_NAME, NEW._IDT, NEW._STATE, TG_OP);
